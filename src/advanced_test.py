...
from typing import List, Dict, Optional, Any   # UP006, F401
import sys
import json
import logging
...
def process_data(items, config):  # ANN001, ANN201
    ...
    for item in items:
        ...
        if item.get('status') == 'active' and item.get('priority') >= 5 and len(item.get('description', '')) > 100 and item.get('category') in ['high', 'medium']:
            results.append(item)
...
class AdvancedProcessor:
    ...
    def __init__(self, name, config_path = None):  # ANN001, ANN204, E251: Default arg spacing
        self.name = name
        self.config = None
    ...
    def load_configuration(self, path):  # ANN001, ANN201
        ...
        try:
            ...
            logging.exception("Config file not found: %s", e)  # TRY401: Redundant exception
    ...
    def transform_data(self, input_data):  # ANN001, ANN201
        ...
        filtered_items = [item for item in input_data.get('items', []) if item.get('active') and item.get('processed_date') and len(item.get('metadata', {})) > 0]
